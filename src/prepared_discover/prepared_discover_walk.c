  ...
  WT_SESSION_IMPL *session, WT_REF *ref, void *context, bool visible_all, bool *skipp)
{
    ...
    *skipp = false; /* Default to reading */
     ...
     * When preserve_prepared is enabled and the page-deleted structure is in the INPROGRESS state,
     * register it in the pending prepared map so the application can claim and resolve it after
     * restart.
     */
    if (WT_REF_GET_STATE(ref) == WT_REF_DELETED &&
      WT_REF_CAS_STATE(session, ref, WT_REF_DELETED, WT_REF_LOCKED)) {
        ...
        if (page_del->prepare_state == WT_PREPARE_INPROGRESS)
            ret = __wti_prepared_discover_add_fast_truncate(
              session, page_del->prepared_id, page_del->prepare_ts);
        WT_REF_SET_STATE(ref, WT_REF_DELETED);
        WT_RET(ret);
    }
    /*
     * Any deleted page can be skipped there is no leaf-page content to discover.
     */
    if (WT_REF_GET_STATE(ref) == WT_REF_DELETED) {
        *skipp = true;
        return (0);
